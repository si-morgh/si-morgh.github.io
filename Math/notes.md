...
{%- for note in sorted_notes %}
 <li>
 <a href="{{ note.link | relative_url }}" class="link-list__title" target="_blank" rel="noopener">{{ note.title }}{% if note.year %}, {{ note.year }}{% endif %}</a>
 {%- if note.size %}<span class="link-list__size">PDF, {{ note.size }}</span>{% endif %}
